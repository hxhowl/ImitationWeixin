...
@interface MeTableVC ()
@property(nonatomic, strong) NSArray *meCellInfoArray;
...
- (void)viewDidLoad {
    ...
    UIImage *image = [UIImage imageNamed:@"tabbar_meHL"];
...
#pragma mark meCellInfoArray的get方法
- (NSArray *)meCellInfoArray{
    if (_meCellInfoArray == nil) {
        NSArray *dictArray = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"MeCellModel" ofType:@"plist"]];
        //HCPLog(@"log1---%@",dictArray);
        NSMutableArray *arr = [NSMutableArray array];
        for (NSDictionary *dict in dictArray) {
            CommonCellModel *meCellInfo = [CommonCellModel CommonCellWithDict:dict];
            [arr addObject:meCellInfo];
        }
        _meCellInfoArray = arr;
        //HCPLog(@"log---%@",arr);
    }
    return _meCellInfoArray;
}
...
{
    if (indexPath.section == 0) {
        static NSString *cellID = @"meInfoCell";
        ...
        if (indexPath.section > 1) {
            for (NSInteger j = 1; j< indexPath.section; j++) {
                n += [tableView numberOfRowsInSection:j];
            }
        }
        cell.commonCellModel = self.meCellInfoArray[n];
        return cell;
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 3 &&indexPath.row == 0 ) {
        SettingTableVC *settingTVC = [[SettingTableVC alloc] initWithStyle:UITableViewStyleGrouped];
